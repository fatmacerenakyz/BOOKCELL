...
public class DefaultBasketRepository implements BasketRepository {
    ...
    private static final String SELECT_BASKETS = "SELECT * FROM public.\"BASKET\";";
    ...
    @Override
    public List<Basket> getByCustomer(Integer customerId) {
    ...
    public List<Basket> getAll() {
        List<Basket> baskets = new ArrayList<>();
        try(Connection connection = connect()){
            ResultSet resultSet = connection.createStatement().executeQuery(SELECT_BASKETS);
            while (resultSet.next()){
                Basket basket = new Basket();
                basket.setId(resultSet.getInt("ID"));
                basket.setCustomerId(resultSet.getInt("CUSTOMER_ID"));
